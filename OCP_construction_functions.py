...
###################################################################################
###################################################################################
#Discrete OCP equation generators SODE
###################################################################################
###################################################################################
###################################################################################

class Direct_continuous_generator:
    '''initially intended for indirect, right now state equations dont substitute u anymore!'''
    def __init__(self,parameters,f_func,rho_func,running_cost_func,mayer_func,g_func):
        self.parameters = parameters
        q_names,vq_names,u_names,lamq_names,lamv_names, vlam_names = "","","","","",""
        kappa_names,v_kappa_names = "","" #not properly used for new approach, lamq,vlam instead
        pq_names, plam_names = "",""
        u_names = ""
        ...
        for tmp in self.parameters["variable_names"]:
            ...
            kappa_names += "kappa_"+tmp +","
            v_kappa_names += "v_kappa_"+tmp +","
        ...
        self.kappa = sympy.Matrix(sympy.symbols(kappa_names))#("v_lambda_r, v_lambda_phi"))
        self.vkappa = sympy.Matrix(sympy.symbols(v_kappa_names))#("v_lambda_r, v_lambda_phi"))
        self.py = sympy.Matrix(sympy.symbols(pq_names+plam_names))
        self.vy = sympy.Matrix(sympy.symbols(vq_names+vlam_names))
        self.f_vec = f_func
    ...
    def state_right_hand_side(self):
        ...
        vq_vec = self.f_vec(q,vq,params) + (self.rho_vec(q,vq,params)@u)                        
        return q_vec, vq_vec
    def adjoint_right_hand_side(self):
        '''return vector of the rhs of the first order adjoint equations of q, for lambda, with signature (lambda, v_lambda)
        in the low-thrust case we thus have (lambda_q, lambda_v, v_lambda_q, v_lambda_v)'''
        q,vq= self.q, self.vq
        lamq,lamv = self.lamq, self.lamv
        u= self.u
        params = self.parameters
        q_vec = vq
        lambda_vec = (-sympy.derive_by_array( self.f_vec(q,vq,params).transpose()@lamv + (self.rho_vec(q,vq,params)@u).transpose()@lamv , q)[:,0,0,:]).tomatrix()
        vlam_vec = -lamq- (sympy.derive_by_array(self.f_vec(q,vq,params).transpose()@lamv ,vq) [:,0,0,:]).tomatrix()                       
        return lambda_vec.evalf(subs=self.usubdict), vlam_vec.evalf(subs=self.usubdict)
    def terminal_cost_transversality(self):
        q,vq= self.q, self.vq
        lamq,lamv = self.lamq, self.lamv
        u= self.u
        params = self.parameters
        term1 = lamq + sympy.derive_by_array(self.mayer_term(q,vq,params),q)[:,0,0].tomatrix()
        term2 = lamv + sympy.derive_by_array(self.mayer_term(q,vq,params),vq)[:,0,0].tomatrix()
        return term1,term2 
    ...
    def control_lagrangian(self,q,lam,vq,vlam,u,params):
        return vlam.T @vq + lam.T@(self.f_vec(q,vq,params) + self.rho_vec(q,vq,params)@u)- self.running_cost(q,u,params) 
    def u_lam_relation(self,q,lam,vq,vlam,u,params):
        control_L = self.control_lagrangian(q,lam,vq,vlam,u,params)
    ...
    def new_Lagrangian(self):
        ...
        u= self.u
        return self.control_lagrangian(q,lam,vq,vlam,u,self.parameters)[0]
    def p_y(self):
        control_L = self.new_Lagrangian()
    ...
    def Pontryagins_Hamiltonian(self):
        ...
        f_eval = self.f_vec(q,vq,np_params)
        rho_eval = self.rho_vec(q,vq,np_params)
        Pontry_H = lamq.transpose()@vq + lamv.transpose()@(f_eval + rho_eval@u)- self.running_cost(q,u,np_params)
        return Pontry_H
    ...
    def u_eval_from_new(self,q,lam,pq,plam,params):
        ...
        u_lambdified = sympy.lambdify(all_var,u_expr)
        return u_lambdified(*q,*lam,*pq,*plam)
        
    def kappa_calc(self,lamv):
        return lamv
    def v_kappa_calc(self,q,vq,lamq,lamv,u):
        DvC = sympy.derive_by_array(self.running_cost(self.q,self.u,self.parameters)[0],self.vq).tomatrix()
        Dvf = sympy.derive_by_array((self.f_vec(self.q,self.vq,self.parameters).T @ self.lamv)[0],self.vq).tomatrix()
        v_kappa = DvC - Dvf - self.lamq
        v_kappa= v_kappa.subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.q,q)]).subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.vq,vq)])
        v_kappa= v_kappa.subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.lamq,lamq)]).subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.lamv,lamv)])
        v_kappa= v_kappa.subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.u,u)])
        return v_kappa
    def lamv_calc(self,kappa):
        return kappa
    def lamq_calc(self,q,vq,kappa,vkappa):
        DvC = sympy.derive_by_array(self.running_cost(self.q,self.u,self.parameters)[0],self.vq).tomatrix()
        Dvf = sympy.derive_by_array((self.f_vec(self.q,self.vq,self.parameters).T @ self.lamv)[0],self.vq).tomatrix()
        lamq_expr = DvC - Dvf - self.vkappa
        lamq_expr= lamq_expr.subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.q,q)]).subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.vq,vq)])
        lamq_expr= lamq_expr.subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.vkappa,vkappa)]).subs([[tmp1,tmp2]for tmp1,tmp2 in zip(self.lamv,kappa)])
        return lamq_expr

    def control_lagrangian_no_u(self,q,lam,vq,vlam,u,params):
        u_vec = self.u
        control_L= self.control_lagrangian(q,lam,vq,vlam,u_vec,params).subs(self.usubdict).subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.q,q) ]).subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.vq,vq) ]).subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.lamv,lam) ]).subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.vlam,vlam) ])
        return control_L
...
###################################################################################
###################################################################################
#Discrete OCP equation generators low-thrust
###################################################################################
###################################################################################
###################################################################################
...
class discrete_standard_direct_eq_generator:
    def __init__(self,PMP_low_thrust_eq ):
        ...
        self.control_L_k_no_u = discrete_control_Lagrangian(self.cont_equations.control_lagrangian_no_u,self.all_vars_new_approach,self.cont_equations.parameters)
        q_d,vq_d,lamq_d,lamv_d,U_d,U1_d,U2_d = [],[],[],[],[],[],[]
        self.mu = sympy.Matrix(sympy.symbols(["mu_" + str(tmp) for tmp in self.cont_equations.q]))
        self.nu = sympy.Matrix(sympy.symbols(["nu_" + str(tmp) for tmp in self.cont_equations.q]))
    ...
    def generate_new_control_objective(self):
        ...
        discrete_objective_function -= sympy.flatten(lam_d[0].transpose()@v0)[0]
        discrete_objective_function += sympy.flatten(lam_d[-1].transpose()@vN)[0]
        #Lagrange terms
    ...
    def vqk1_p_no_u(self):
        return sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k1"][1])
    def vqk_m_no_u(self):
        return -sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k"][1])
    
    def vlamk1_p_no_u(self):
        return sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k1"][0])
    def vlamk_m_no_u(self):
        return -sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k"][0])
    def p_y_k1p_no_u(self):
            p_lam_k1=sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k1"][1])
            p_q_k1 =sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k1"][0])
            return sympy.Matrix(sympy.flatten(p_q_k1)+ sympy.flatten(p_lam_k1))
    def p_y_km_no_u(self):
            p_lam_k=-sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k"][1])
            p_q_k =-sympy.derive_by_array(sympy.flatten(self.control_L_k_no_u["discrete_control_L"])[0], self.control_L_k_no_u["vars_k"][0])
            return sympy.Matrix(sympy.flatten(p_q_k)+ sympy.flatten(p_lam_k))

    def p_v_d_from_y_d_no_u(self,q_d,lam_d,np_params):
        all_vars_to_sub = [q_d,lam_d]
        p_y_d = []
        p_y_k1p = self.p_y_k1p_no_u().subs("gamma",np_params["gamma"]).subs("alpha",np_params["alpha"]).subs("h",np_params["h"])
        p_y_km = self.p_y_km_no_u().subs("gamma",np_params["gamma"]).subs("alpha",np_params["alpha"]).subs("h",np_params["h"])
        k1_vars = self.control_L_k_no_u["vars_k1"][:-2]
        k_vars = self.control_L_k_no_u["vars_k"][:-2]
        for i in range(len(q_d)-1):
            p_y_i = p_y_km
            for j in range(len(k_vars)):
                p_y_i = self.__k_to_i_eval(p_y_i,k_vars[j],all_vars_to_sub[j][i])
        ...
        return p_y_d     
    def generate_new_control_objective_no_u(self):
        '''generate here the new control objective'''


        # discrete_objective_function = terminal_cost

        k1_vars = self.control_L_k_no_u["vars_k1"]
        k_vars = self.control_L_k_no_u["vars_k"]

        v_k_m = self.vqk_m_no_u()
        v_k1_p = self.vqk1_p_no_u()
        v0 = v_k_m.tomatrix()
        all_d = self.all_d_new_approach
        q_d,lam_d,U1_d,U2_d = self.all_d_new_approach
        for i in range(len(self.all_d_new_approach)):
        ...
        # print( sympy.flatten(self.control_L_k["discrete_control_L"]))
        for j in range(self.cont_equations.parameters["N"]):
            discrete_control_L_eval = self.control_L_k_no_u["discrete_control_L"]
            for i in range(len(self.all_d_new_approach)):
                discrete_control_L_eval = self.__k_to_i_eval(discrete_control_L_eval,k1_vars[i],all_d[i][j+1])
                discrete_control_L_eval = self.__k_to_i_eval(discrete_control_L_eval,k_vars[i],all_d[i][j])
            discrete_objective_function -= sympy.flatten(discrete_control_L_eval)[0]
    ...
    def calc_KKT_new_no_u(self):
        objective_and_vars = self.generate_new_control_objective_no_u()
        objective = objective_and_vars["discrete_objective"]
        mu_eq = sympy.flatten(sympy.derive_by_array(objective,self.mu))
        nu_eq = sympy.flatten(sympy.derive_by_array(objective,self.nu))
        rest_eqs = []
...
#         # vq_as_func_disc_p = sympy.solve(p_mech_cont  - self.cont_equations.pq, self.cont_equations.vq)
#         # vq_discrete = sympy.Matrix([vq_as_func_disc_p[tmp] for tmp in self.cont_equations.vq])
#         # vq_k1_p = vq_discrete.subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.cont_equations.pq,p_mech_k1_p)])
#         # vq_k_m = vq_discrete.subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.cont_equations.pq,p_mech_k_m)])
#         vq_k1_p = self.vqk1_p()
#         vq_k_m = self.vqk_m()
...
#         # p_mech_N = p_mech_cont.subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.cont_equations.q,q_d[-1] )]).subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.cont_equations.vq,vN )])
#         # p_mech_0 = p_mech_cont.subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.cont_equations.q,q_d[0] )]).subs([[tmp1,tmp2] for tmp1,tmp2 in zip(self.cont_equations.vq,v0 )])
#         #mayer term    
#         discrete_objective_function = sympy.flatten(self.cont_equations.mayer_term(q_d[-1],vN,self.cont_equations.parameters))[0]
#         # initial constraints
#         discrete_objective_function += sympy.flatten(self.mu.transpose()@(q_d[0]- self.cont_equations.parameters["q0"])    )[0]
...
#         dlam0term = sympy.flatten(self.nu - dlam0term_element)
#         dlamTterm = sympy.flatten(objective_and_vars["D2phi"] + dlam0term_element)
#         print(dlam0term)
#         print(dlamTterm)
#         # dlam0term = sympy.flatten(self.nu - self.all_d_new_approach[1][0])
#         # dlamTterm = sympy.flatten(objective_and_vars["D2phi"]+ self.all_d_new_approach[1][-1])
